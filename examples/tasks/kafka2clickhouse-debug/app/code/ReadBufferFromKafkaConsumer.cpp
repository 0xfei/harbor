...
{
    extern const Event ETLErrorDropMessage;
    extern const Event ETLKafkaErrorAssign;
}
        ...
        if (new_messages.empty())
        {
            ...
            if (assignment.empty())
            {
                ...
                else
                {
                    ...
                    ProfileEvents::increment(ProfileEvents::ETLKafkaErrorAssign);
...
size_t ReadBufferFromKafkaConsumer::filterMessageErrors()
{
    ...
    auto new_end = std::remove_if(messages.begin(), messages.end(), [this](auto & message)
    {
        if (auto error = message.get_error())
        {
            ...
            ProfileEvents::increment(ProfileEvents::ETLErrorDropMessage);
            return true;
        }
        return false;
    });
