...
void MergeTreePartition::initializeIdAndVersion()
{
    if (value.size() != 5)
        throw Exception(
            ErrorCodes::LOGICAL_ERROR,
            "Invalid partition key size: {}. It should be 5 when enable_universal_partition_scheme == true",
            value.size());

    dt = value[0].safeGet<UInt64>();
    ty = value[1].safeGet<Int64>();
    tag = value[2].safeGet<String>();
    cell = value[3].safeGet<Int64>();
    ver = value[4].safeGet<Int64>();
    ...
    void MergeTreePartition::load(const MergeTreeData & storage, const PartMetadataManagerPtr & manager)
{
    ...
    if (storage.getSettings()->enable_universal_partition_scheme)
        initializeIdAndVersion();
}

void MergeTreePartition::store(const MergeTreeData & storage, const DiskPtr & disk, const String & part_path, MergeTreeDataPartChecksums & checksums, bool sync) const
{
    auto metadata_snapshot = storage.getInMemoryMetadataPtr();
    const auto & partition_key_sample = adjustPartitionKey(metadata_snapshot, storage.getContext()).sample_block;
    store(partition_key_sample, disk, part_path, checksums, sync);
}

void MergeTreePartition::store(const Block & partition_key_sample, const DiskPtr & disk, const String & part_path, MergeTreeDataPartChecksums & checksums, bool sync) const
{
...
void MergeTreePartition::create(
    const StorageMetadataPtr & metadata_snapshot, Block block, size_t row, ContextPtr context, bool enable_universal_partition_scheme)
{
    if (!metadata_snapshot->hasPartitionKey())
        return;
    ...
    if (enable_universal_partition_scheme)
        initializeIdAndVersion();
}
...
const NameSet MergeTreePartition::FILES_TO_SKIP_DURING_MUTATION
    = {"checksums.txt", "minmax__time.idx", "minmax__type.idx", "minmax__tag.idx", "minmax__cell.idx", "minmax__ver.idx", "partition.dat"};
