...
using ReadTaskCallback = std::function<String()>;
    ...
    struct MergeTreeMetadataCacheParams {
        size_t size;
        size_t max_background_flushes;
        size_t max_background_compactions;
        size_t max_file_opening_threads;
    };
#endif
...
private:
    ...
    String kuaishou_auth_token_hashed;
    KuaishouAuthTokenCheckResult kuaishou_auth_token_check_result = SKIPPED;
    bool kuaishou_auth_token_required = false;
    bool rewrite_by_remote_query = false;
    ...
    VisibilityMap main_visibility_map;
    Block main_visibility_map_block;
    VisibilityMapsAndBlocks visibility_maps_and_blocks;
    VisibilityMaps visibility_maps;
    ...
    struct QueryAccessInfo
    {
        ...
        std::set<std::string> columns_in_where{};
...
public:
    ...
    void setUser(const Poco::Net::SocketAddress & address);
    ...
    void addMainVisibilityMap(const Block & block);
    const VisibilityMap & getMainVisibilityMap() const;
    const VisibilityMap * getVisibilityMapForTable(const String & full_name) const;
    const Block & getMainVisibilityMapBlock() const;
    bool hasVisibilityMap(const String & table_name) const;
    const Block & addVisibilityMapIfNotExistAndGetBlock(const String & table_name, const VisibilityMapPtr & visibility_map);
    void addVisibilityMapsFromMaps(VisibilityMaps maps);
    void addVisibilityMapsFromBlock(const Block & block);
    Block getVisibilityMapsBlock() const;
    void setKuaishouAuthTokens(const std::vector<std::string> & tokens);
    std::pair<KuaishouAuthTokenCheckResult, String> checkKuaishouAuthToken(const String & user_token) const;
    void setKuaishouAuthTokenEnable(bool enable);
    bool getKuaishouAuthTokenEnable() const;
    ...
    void addQueryAccessInfo(const std::unordered_map<std::string, std::string> & table_partitions);

    void addQueryColumnInfo(const std::string & column_in_where);
    ...
    String getKuaishouAuthTokenHashed() const { return kuaishou_auth_token_hashed; }
    void setKuaishouAuthTokenHashed(String hashed) { kuaishou_auth_token_hashed = hashed; }

    KuaishouAuthTokenCheckResult getKuaishouAuthTokenCheckResult() const { return kuaishou_auth_token_check_result; }
    void setKuaishouAuthTokenCheckResult(KuaishouAuthTokenCheckResult check_result) { kuaishou_auth_token_check_result = std::move(check_result); }

    bool getKuaishouAuthTokenRequired() const { return kuaishou_auth_token_required; }
    void setKuaishouAuthTokenRequired(bool required) { kuaishou_auth_token_required = required; }

    bool getRewriteByRemoteQuery() const { return rewrite_by_remote_query; }
    void setRewriteByRemoteQuery(bool rewrite) { rewrite_by_remote_query = rewrite; }
    ...
    void setMainTable(StorageID db_and_table) { main_table = std::move(db_and_table); }
    ...
    void setServerId(const String & id);
    ...
    std::map<String, std::shared_ptr<zkutil::ZooKeeper>> getAuxiliaryZooKeeperMap() const;

    String getDefaultZookeeperName() const;
    ...
    ThreadPool & getStartCheckPool() const;
    std::latch & getStartCheckLatch() const;
    std::atomic_bool & getStartCheckcancelled() const;
    ...
    /// Has mvcc configuration or not.
    bool hasMVCC() const;
    void setMVCCWorker(std::unique_ptr<MVCCWorker> mvcc_worker);
    MVCCWorker & getMVCCWorker() const;

    const String & getZookeeperClusterPath() const;
    void setZookeeperClusterPath(const String & zk_cluster_path);
...
#if USE_ROCKSDB
        void initializeMergeTreeMetadataCache(const String & dir, MergeTreeMetadataCacheParams params, size_t delete_cache_fail_threshold);
#endif
    ...
    std::shared_ptr<FunctionABTestLog> getFunctionABTestLog() const;
...
private:
    ...
    void reloadExplicitDistributedTables(const Clusters & clusters);
};
