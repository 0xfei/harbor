...
> 本项目收录了用于考察 **顶级 Coding Agent** 综合能力的系统级编程评测题。  
...
| # | 任务目录 | 难度 | 类别 | 详情 |
|---|----------|------|------|------|
| 1 | [stream-window-aggregator](./examples/tasks/stream-window-aggregator/TASK.md) | Hard | 系统编程 / 并发 | 流式滑动窗口聚合引擎，lock-free + 精确事件时间语义 |
| 2 | [bitmap-vector-fix](./examples/tasks/bitmap-vector-fix/TASK.md) | Medium | 调试 / C++ | 修复 `BitMapManager` 中 `grow()` 的 3 个典型 bug |
| 3 | [stream-ingest-deadlock](./examples/tasks/stream-ingest-deadlock/TASK.md) | Hard | 系统编程 / Python | 修复多线程数据摄入系统中的死锁问题 |
| 4 | [distributed-chaos-system](./examples/tasks/distributed-chaos-system/TASK.md) | Extreme | 分布式系统 / 非确定性 | 修复分布式计费系统的非确定性行为 |
...
### 1. stream-window-aggregator

**测试方式：**
```bash
uv run harbor run -a oracle -p examples/tasks/stream-window-aggregator  # 预期 1.0
uv run harbor run -a nop -p examples/tasks/stream-window-aggregator      # 预期 0.0
```
...
| 指标 | 结果 |
|------|------|
| Harbor 真实运行 | 0/10 次通过编译 |
| API 生成代码检查 | 编译失败（出现 `__builtin_ia32_pause`） |

**失败原因：**
1. **隐藏细节陷阱**：Event struct 需要 8-byte padding 使 sizeof(Event)==24，模型注意力被复杂约束分散
2. **平台差异**：生成代码出现 x86 builtin，在 aarch64/clang 环境编译失败
3. **约束过多**：禁锁、禁 map、内存限制、线程数限制、事件时间语义等，模型难以同时满足

**难度验证：** ✅ 该题有效卡住了 kimi-k2.5
...
**测试方式：**
```bash
uv run harbor run -a oracle -p examples/tasks/bitmap-vector-fix  # 预期 1.0
uv run harbor run -a nop -p examples/tasks/bitmap-vector-fix    # 预期 0.0
```
...
| 指标 | 结果 |
|------|------|
| Harbor 真实运行 | 43/43 tests passed |
| 修复质量 | 代码简洁，符合预期 |

**成功原因：**
1. **bug 典型**：`reserve()` vs `resize()`、`size_t` 下溢、越界访问都是教科书级错误
2. **代码量小**：单文件 ~60 行，模型容易理解完整上下文
3. **问题明确**："修复 grow() 函数中的 bug" 指向清晰

**难度验证：** ⚠️ 题目偏简单，需要增加更多边界条件测试

---

### 3. stream-ingest-deadlock

**测试方式：**
```bash
uv run harbor run -a oracle -p examples/tasks/stream-ingest-deadlock  # 预期 1.0
uv run harbor run -a nop -p examples/tasks/stream-ingest-deadlock    # 预期 TIMEOUT
```
...
| 问题识别 | ✅ 能识别双锁嵌套问题 |
| API 生成代码 | ❌ 仍超时（修复不完整） |
| 修复方案 | 使用单锁，但可能有其他并发问题 |

**部分失败原因：**
1. **并发复杂性**：死锁问题需要精确理解 Queue 的线程安全性
2. **约束理解**："保持 Queue 操作"不等于"Queue 需要外部锁"
3. **测试敏感**：任何并发瑕疵都会导致超时
...
### 4. distributed-chaos-system

**测试方式：**
```bash
uv run harbor run -a oracle -p examples/tasks/distributed-chaos-system  # 预期 1.0
uv run harbor run -a nop -p examples/tasks/distributed-chaos-system     # 预期 0.0（非确定性）
```

**kimi-k2.5 评分：> 50%（条件触发）**

| 测试场景 | 结果 |
|---------|------|
| 隐晦问题描述 | ❌ 使用 hash 方案过度复杂化 |
| 明确 root cause | ✅ 正确修复所有文件 |

**成功条件：**
- 当明确告知"随机性在 mutation.py/commit_graph.py/replay.py"时，kimi-k2.5 能正确修复：
  ```python
  # mutation.py
...
**失败条件：**
- 隐晦描述时，模型倾向于复杂方案（如基于 hash 的伪随机）

**难度验证：** ✅ Extreme 级别合理，依赖问题表述清晰度
...
| 项目 | 难度 | kimi-k2.5 成功率 | 验证状态 |
|------|------|-----------------|---------|
| stream-window-aggregator | Hard | 0% | ✅ 有效卡住 |
| bitmap-vector-fix | Medium | 100% | ⚠️ 偏简单 |
| stream-ingest-deadlock | Hard | < 30% | ✅ 有效卡住 |
| distributed-chaos-system | Extreme | > 50%* | ✅ 条件触发 |

*distributed-chaos-system 成功率依赖问题描述清晰度

### 关键发现

**1. 模型弱项**
- **隐藏细节**：padding、alignment 等内存布局问题
- **多约束并发**：同时满足 5+ 个约束时容易遗漏
- **平台差异**：生成 x86 specific 代码
- **并发语义**：精确理解锁语义、Queue 线程安全性

**2. 模型强项**
- **典型 bug**：教科书级错误（reserve/resize、size_t 下溢）
- **小代码库**：单文件 < 100 行时表现好
- **明确指向**："修复 X 文件中的 Y 问题"

**3. 题目设计原则**
- ✅ 隐藏关键细节（如 padding）能有效增加难度
- ✅ 多文件分散 bug 能测试跨模块理解
- ⚠️ 过于典型的 bug 会被秒杀
- ✅ 约束语义模糊（"保持 retry"vs"必须失败"）能测试理解能力

### 后续优化方向

1. **bitmap-vector-fix**：增加更多边界测试（空输入、超大维度、并发访问）
2. **stream-ingest-deadlock**：添加 ordering 验证
3. **新增题目**：参考「题目方向规划」章节，优先实现 Level 5 黑盒诊断题
...
**Level 6（高强度验证）：**
- [ ] `apache-analyser` — 目标是跑通主流 apache 项目，跟踪 github 修复记录，验证 ai 能否完成同样的任务
- [ ] `data-agent` — 跨各类数据库的数据迁移、sql 优化等专项任务
